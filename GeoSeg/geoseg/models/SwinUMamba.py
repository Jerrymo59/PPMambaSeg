...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2], 
                 ...
                 use_checkpoint=False, pretrained=True, **kwargs):
        super().__init__()
        ...
        self.res_layers.append(self.backbone.layer1)
        self.res_layers.append(self.backbone.layer2)
        self.res_layers.append(self.backbone.layer3)
        self.res_layers.append(self.backbone.layer4)
    ...
    def forward(self, x):
        ...
        res_in_channels = [64, 64, 128, 256]
        vss_out_channels = [96, 192, 384, 768]
        x_ret = []
        x_ret.append(x)
        res_input = []
        ...
        for s, layer in enumerate(self.layers):
            ...
            if s in [1, 2]:
                ...
                add_channel =nn.Conv2d(in_channels=vss_out_channels[s-1],out_channels=res_in_channels[s-1],kernel_size=1)
                add_channel.to(device)
                de_channel = nn.Conv2d(in_channels=res_in_channels[s], out_channels=vss_out_channels[s],
                                    kernel_size=1)
                max_pool = nn.MaxPool2d(2, 2)
                de_channel.to(device)
                res_input[s-1] = res_input[s-1].permute(0, 3, 1, 2)
                x_permute = res_input[s-1]

                x_permute = add_channel(res_input[s-1])

                x_permute = self.res_layers[s-1](x_permute)

                x_permute = de_channel(x_permute)

                if s < 2:
                    x_permute = max_pool(x_permute)

                x_permute = x_permute.permute(0, 2, 3, 1)
...
def load_pretrained_ckpt(
    model, 
    ckpt_path="geoseg/pretrain/vmamba_tiny_e292.pth"
):
