...
import timm
import re
from geoseg.models.SwinUMamba import *
...
##PPpooling
class PPooling(nn.Module):
    def __init__(self, dim=128, last_feat_size=16):
        super().__init__()
        ...
        self.pool_layers.append(nn.Sequential(
            ConvBNReLU(dim, dim//4, kernel_size=1),
    ...
    def forward(self, x):  # B, C, H, W
        ...
        weights = nn.ReLU()(self.weights)
        fuse_weights = weights / (torch.sum(weights, dim=0) + self.eps)
        x = fuse_weights[0] * res + fuse_weights[1] * ppm_out
        return x
...
class PPMamba(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            self.vss_layers.append(layer)

        encoder_channels = self.backbone.feature_info.channels()

        self.decoder = Decoder(encoder_channels, decode_channels, dropout, window_size, num_classes)

        self.ppooling = nn.ModuleList()
        last_feat_size = [1024, 512, 128, 64]
        for i_layer in range(self.num_layers):
            self.ppooling.append(PPooling(dims[i_layer], last_feat_size[i_layer]))
